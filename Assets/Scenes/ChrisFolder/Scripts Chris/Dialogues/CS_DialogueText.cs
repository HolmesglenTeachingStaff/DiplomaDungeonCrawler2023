...
public class CS_DialogueText : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(canTalk == true)
        {
            ...
            if (Input.GetKeyDown(KeyCode.G) && HasDisplayed == false)
            {
                messageCount++;
                StopAllCoroutines();
            ...
            if(Input.GetKeyDown(KeyCode.G) && HasDisplayed == true)
            {
                StopAllCoroutines();
    ...
    {
        textUI.text = "";
        anim.Play("CS_isGesturing"); // anim to indicate talking
